...
const introspectEndpoints: Endpoint[] = [
	{
		...
		method: 'POST',
		note: 'Body: token=<access or refresh token>, token_type_hint=access_token (optional). Client auth: client_secret_basic, client_secret_post, or private_key_jwt — the same auth the token endpoint accepts.',
		path: '{oidcRoute}/introspect'
	}
];

const revokeEndpoints: Endpoint[] = [
	{
		description:
			'Voluntarily kill a refresh token, e.g. on "Sign out of this device".',
		method: 'POST',
		note: 'Body: token=<refresh or access token>, token_type_hint=refresh_token (optional). Same client auth as /token + /introspect.',
		path: '{oidcRoute}/revoke'
	}
];
			...
			<div style={mainContentStyle(isMobileOrTablet)}>
				<animated.div style={heroGradientStyle(themeSprings)}>
					...
					<p style={paragraphLargeStyle}>
						...
						&lt;yourapp&gt;&quot;. authorization_code + mandatory
						PKCE, ES256 JWTs signed by a key you own (self-hosted
						JWKS), refresh-token rotation, DPoP sender-constrained
						tokens, and a complete OAuth2/OIDC surface (introspect,
						revoke, device flow, PAR, JAR, DCR, private_key_jwt,
						RP-initiated + back-channel logout, userinfo).
					</p>
				</animated.div>
				...
				<section style={sectionStyle}>
					...
					<p style={paragraphSpacedStyle}>
						Configure the <code>oidc</code> block with a signing
						key, a relying-party client registry, and the code /
						refresh stores. The authorize endpoint reuses the
						package session, so the IdP login gets passkeys / MFA /
						SSO for free.
					</p>
					<PrismPlus
				...
				<section style={sectionStyle}>
					...
					<p style={paragraphSpacedStyle}>
						The OIDC <code>/userinfo</code> endpoint lets RPs swap a
						valid access token for the user&apos;s profile claims —
						the missing piece that lets <em>any</em> generic OIDC
						client (Grafana, Tailscale, ArgoCD, etc.) onboard.
						Configure <code>getUserInfo(sub)</code> and the package
						mounts both GET and POST, with proper{' '}
						<code>WWW-Authenticate: Bearer</code> challenges on
						invalid tokens.
					</p>
					<PrismPlus
				...
				<section style={sectionStyle}>
					...
					<p style={paragraphSpacedStyle}>
						OIDC defines four standard ways for an RP to ask for a
						fresher or different login — <code>prompt</code>,{' '}
						<code>max_age</code>, <code>id_token_hint</code>, and
						(combined with the bot/abuse abuse and adaptive blocks){' '}
						<code>acr_values</code>. The authorize endpoint honors
						them against the session&apos;s <code>auth_time</code>{' '}
					...
					<Callout themeSprings={themeSprings} variant="note">
						All four are honored automatically. Pair with{' '}
						<code>requireRecentAuth</code> on sensitive RP actions
						for end-to-end freshness guarantees.
					</Callout>
				</section>
				...
				<section style={sectionStyle}>
					...
					<p style={paragraphSpacedStyle}>
						The strongest client authentication (RFC 7521/7523). The
						client signs a JWT assertion with its private key; the
						AS holds only the public JWK. No shared secret means
						stolen logs can&apos;t leak credentials. The same wiring
						also accepts <code>client_secret_jwt</code> (HS256). A{' '}
						<code>jti</code> replay store enforces RFC 7523 §3
						one-time use.
					</p>
					<PrismPlus
				...
				<section style={sectionStyle}>
					...
					<p style={paragraphSpacedStyle}>
						Partners self-register at runtime without you minting
						clients by hand. Gate registration with an initial
						access token; the registered client receives a{' '}
				...
				<section style={sectionStyle}>
					...
					<p style={paragraphSpacedStyle}>
						...
						<code>request_uri</code> to hand to{' '}
						<code>/authorize</code>. Defeats browser-leak attacks on
						long query strings; required by FAPI profiles.
						Per-client opt-in via{' '}
				...
				<section style={sectionStyle}>
					...
					<p style={paragraphSpacedStyle}>
						Pass the entire <code>/authorize</code> request as a
						signed JWT (<code>?request=&lt;jwt&gt;</code>), proving
						the parameters originated from the registered client.
						Required by FAPI / Open Banking / FHIR; per-client
						opt-in via <code>requireSignedRequestObject</code>.
						Shares the same JWKS infrastructure as{' '}
				...
				<section style={sectionStyle}>
					...
					<p style={paragraphSpacedStyle}>
						DPoP (RFC 9449) binds the access token to the
						client&apos;s proof key (<code>cnf.jkt</code>), so a
						stolen bearer token is useless without the private key —
						WorkOS doesn&apos;t offer this. It&apos;s automatic at
						the token endpoint; verify the proof on your resource
						server with <code>verifyDpopProof</code>. §8 nonces opt
						in via a <code>dpopNonceStore</code> to defeat
						pre-computed-proof replays. RFC 9470{' '}
						<code>acr_values</code> pair with the adaptive + MFA
						gates to force step-up before issuing a code.
					</p>
					<PrismPlus
						codeString={oidcDpop}
						language="typescript"
				...
				<section style={sectionStyle}>
					...
					<p style={paragraphSpacedStyle}>
						The 2026 frontier. An agent trades a user&apos;s token
						for a narrower, short-lived, audience-bound{' '}
						<strong>on-behalf-of</strong> token via{' '}
						<strong>RFC 8693 token exchange</strong> — the issued
						token keeps the user&apos;s <code>sub</code>, binds{' '}
						<code>aud</code> to the resource (
						<strong>RFC 8707</strong>), and records the delegation
						in an <code>act</code> claim (DPoP-bindable).{' '}
						<code>mcpProtectedResourceMetadata</code> emits the MCP
						/ RFC 9728 discovery doc. Matches Auth0&apos;s
						&quot;Auth for MCP&quot;.
					</p>
					<PrismPlus
