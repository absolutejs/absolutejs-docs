...
export const SitemapExcludeRoutesSection = ({ themeSprings }: ThemeProps) => (
	<section style={sectionStyle}>
		...
		<p style={paragraphSpacedStyle}>
			...
			<code>absolute.config.ts</code> to keep routes out of the sitemap.
		...
		<ul style={listStyle}>
			<li style={listItemStyle}>
				...
				<code>**</code> segments in their path (dynamic, can't
				enumerate).
			</li>
			<li style={listItemStyle}>
			...
			<li style={listItemStyle}>
				Pure <code>redirectTo</code> routes inside an SPA router config
				— they aren't destination URLs.
			</li>
		</ul>
		<p style={paragraphSpacedStyle}>
			Wildcard routes <em>are</em> kept when their handler is a page
			handler — they're the mount points for SPA sub-routes. The wildcard
			URL itself isn't emitted (it has no canonical destination); the
			sitemap emits the SPA's actual leaf URLs instead.
		</p>
	</section>
);
