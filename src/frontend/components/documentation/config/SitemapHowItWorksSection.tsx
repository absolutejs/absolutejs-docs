			...
			steps={[
				{
					description: (
						<>
							walks every <code>GET</code> route registered on
							your Elysia app and inspects each handler's source
							for a call to one of the{' '}
							<code>handle*PageRequest</code> helpers (
							<code>handleAngularPageRequest</code>,{' '}
							<code>handleReactPageRequest</code>, etc.).
						</>
					),
					title: 'Discovers page routes'
				},
				{
					description: (
						<>
							for any wildcard page route like{' '}
							<code>/portal/*</code>, it statically parses the
							SPA's framework router config (Angular's{' '}
					...
					title: 'Walks framework routers'
				},
				{
					description: (
						<>
							extracts any literal{' '}
							<code>{'sitemap: { ... }'}</code> option you pass to
							a <code>handle*PageRequest</code> call and applies
							it to that route's sitemap entries.
						</>
					),
					title: 'Reads per-page metadata'
				},
				{
					description: (
						<>
							parameterised routes (<code>:slug</code>,{' '}
							<code>**</code>), redirects, and routes explicitly
							opted out via{' '}
							<code>data.sitemap === 'exclude'</code> (or each
							framework's equivalent) are dropped.
						</>
					),
					title: 'Filters non-emittable URLs'
				},
				{
					description:
						'generates the XML and writes it to the build directory where the static file server picks it up.',
					title: 'Writes sitemap.xml'
				}
			]}
			themeSprings={themeSprings}
		/>
