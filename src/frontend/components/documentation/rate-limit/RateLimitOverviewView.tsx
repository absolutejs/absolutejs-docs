	...
	>
		<animated.div style={heroGradientStyle(themeSprings)}>
			...
			<p style={paragraphLargeStyle}>
				A first-class, from-scratch, 2026 rate limit for Bun + Elysia.{' '}
				<strong>GCRA</strong> by default — the algorithm Stripe uses —
				with token bucket and sliding-window also bundled. IETF draft-09{' '}
				<code>RateLimit-*</code> headers, IPv6 grouping by{' '}
				<code>/64</code>, <code>X-Forwarded-For</code> trust modes,
				BigInt nanosecond TAT, and a lazy-TTL LRU memory store with no
				background sweeper.
			</p>
		</animated.div>
		...
		<section style={sectionStyle}>
			...
			<p style={paragraphSpacedStyle}>
				<code>rateLimit()</code> is an Elysia plugin — wire it with{' '}
				<code>.use()</code> and every request is gated before it reaches
				your handlers. The default key is the requester's IP; the
				default algorithm is GCRA.
			</p>
			<PrismPlus
			...
			<p style={paragraphSpacedStyle}>
				...
				<code>Retry-After</code> + the IETF <code>RateLimit-*</code>{' '}
				headers. The remaining count stays current on the{' '}
				<code>200</code> responses too — so clients can throttle
				themselves before they get refused.
			</p>
		</section>
		...
		<section style={sectionStyle}>
			...
			<p style={paragraphSpacedStyle}>
				Pick GCRA unless you have a reason not to. It's exact, O(1)
				memory per key (one BigInt — the TAT), no boundary effects, no
				float drift. Token bucket and sliding-window are bundled for
				when you specifically want their semantics (burst-then-throttle
				or N-in-the-last-M-seconds, respectively). Stack any of them
				with <code>combined</code>.
			</p>
			<PrismPlus
			...
			>
				<li>
					<strong>GCRA</strong> — exact, BigInt nanosecond TAT, one
					number of state per key, no boundary effects. The default.
				</li>
				<li>
					<strong>Token bucket</strong> — classic. Allows brief bursts
					at refill boundaries. Pick this when you want "fill the
					bucket, fire it all at once" semantics.
				</li>
				<li>
					<strong>Sliding window</strong> — counter approximation.
					Easy to explain on a status page ("you have N requests left
					in the last M seconds").
				</li>
				<li>
					<strong>Combined</strong> — composes any number of
					algorithms; passes only when every component passes. Stack a
					per-IP minute cap with a per-user-id daily cap in one
					plugin.
				</li>
			</ul>
		</section>
		...
		<section style={sectionStyle}>
			...
			<p style={paragraphSpacedStyle}>
				...
				<code>cost</code> function. The bucket goes into overdraft on a
				cost-N hit (you wait it off — same as Stripe's metered
				approach). Key derivation is a string preset (<code>'ip'</code>{' '}
				or <code>'authorization'</code>) or a function.{' '}
				<code>skip</code> bypasses the limit entirely;{' '}
				<code>onAllow</code> and <code>onLimit</code> are symmetric
				hooks for metrics + custom responses.
			</p>
			<PrismPlus
		...
		<section style={sectionStyle}>
			...
			<p style={paragraphSpacedStyle}>
				...
				<code>reset</code> together, plus <code>RateLimit-Policy</code>{' '}
				describing the policy. Set <code>headers: 'legacy'</code> for
				the older <code>X-RateLimit-*</code> form (GitHub circa 2014),{' '}
				<code>'both'</code> for a transition period, or{' '}
				<code>false</code> to suppress everything except Retry-After.
			</p>
			<PrismPlus
		...
		<section style={sectionStyle}>
			...
			<p style={paragraphSpacedStyle}>
				...
				<code>X-Forwarded-For</code> blindly (spoofable) or ignore it
				entirely (broken behind any CDN). <code>trustedProxies: N</code>{' '}
				honors only the last N hops — anything to the left is
				attacker-supplied and ignored. <code>0</code> disables XFF;{' '}
				<code>1</code> is the right default behind a single CDN/LB.
			</p>
			<p style={paragraphSpacedStyle}>
				IPv6 addresses are grouped by their <code>/64</code> prefix by
				default — one user's RIR allocation gets one quota, not one
				quota per device. Configurable via <code>ipv6Prefix</code>. CDN
				headers (<code>cf-connecting-ip</code>,{' '}
				<code>fly-client-ip</code>,<code> true-client-ip</code>,{' '}
				<code>x-real-ip</code>) are honored when XFF is missing and a
				proxy is trusted.
			</p>
			<PrismPlus
		...
		<section style={sectionStyle}>
			...
			<p style={paragraphSpacedStyle}>
				...
				anywhere you need exact rate enforcement without HTTP. Each
				algorithm exposes <code>peek()</code> for status-page read-only
				inspection and <code>reset()</code> for admin clears.
			</p>
			<PrismPlus
