...
export const tunnelArchitecture = `\
                    public HTTPS / WSS                control WS (dialed out
 Twilio / Stripe  ───────────────────▶  RELAY  ◀───────────────────────────  your laptop
  (the internet)                       (public)   request/response + ws mux    'bun run dev'
                                                                                     │
                                                                                     ▼
                                                                            your local server
                                                                              (localhost:3000)

# The relay is the only public piece. It holds ONE control WebSocket that
# your dev machine opens (outbound, so no inbound firewall rules needed).
# Every public request is serialized over that socket, replayed against
# your local server, and the response streamed back. WebSocket connections
# are multiplexed over the same control socket by id.`;
...
# The 'Public:' URL routes straight to your local server. Use it for any
# webhook provider's callback URL — every request hits your live code with
# full HMR, breakpoints, and logs.`;
