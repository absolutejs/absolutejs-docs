...
export const telemetryPlugin = (db: DatabaseType) =>
	new Elysia({ prefix: '/api' })
		...
		.group('/v1/telemetry', (app) =>
			app
				.get('/kpi-summary', ({ protectRoute, status }) =>
					protectRoute(
						async (user) => {
				...
				.get('/versions', async ({ protectRoute, status }) =>
					protectRoute(
						async (user) => {
							if (!whitelistedAdmins.includes(user.auth_sub)) {
								return status(
				...
				.get('/bun-versions', async ({ protectRoute, status }) =>
					protectRoute(
						async (user) => {
							if (!whitelistedAdmins.includes(user.auth_sub)) {
								return status(
				...
				.get(
					...
					({ query, protectRoute, status }) =>
						protectRoute(
							async (user) => {
								if (
									!whitelistedAdmins.includes(user.auth_sub)
								) {
									return status(
										HTTP_STATUS_FORBIDDEN,
										'Access denied'
					...
					({ params: { eventId }, protectRoute, status }) =>
						protectRoute(
							async (user) => {
								...
								const deleted = await deleteTelemetryEvent(
									db,
									eventId
								);
								if (!deleted)
					...
					({ body, protectRoute, status }) =>
						protectRoute(
							async (user) => {
								...
								const deleted =
									await deleteTelemetryEventsByFilter(db, {
